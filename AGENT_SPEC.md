# The Garden: Sudoku & Puzzles — Agent Reference Spec
> **Version:** 1.1  
> **Last updated:** 2026-05-09  
> **Execution environment:** Claude Code + Warp via GitHub  
> **Repo structure assumed:** flat HTML files at root, `assets/` for icons/images, `docs/` for this spec
...
| Primary game | Sudoku |
| Platform targets | Android (primary), iOS (simultaneous if feasible) |
| Tech stack | Vanilla HTML/CSS/JS → Capacitor wrapper |
| Backend | Supabase (not yet created — setup steps included) |
...
├── wordle.html             # Wordle (disabled/coming soon)
├── words.txt               # Wordle word bank
├── privacy-policy.html     # Required for store submission
├── assets/
│   ├── icons/              # Custom SVG icon set (see Phase 2)
│   ├── app-icon/           # Store icons at required sizes
│   └── splash/             # Launch screen assets
├── docs/
│   └── AGENT_SPEC.md       # This file
├── capacitor.config.json   # Added in Phase 6
├── package.json            # Added in Phase 6
└── .github/
    └── workflows/
        ├── deploy-web.yml
        └── build-android.yml
...
Remove **Baby** level. It is too easy to be meaningful. Rename levels and adjust clue counts:
...
### 2.1 Decision: Custom SVG Set

Lucide and Phosphor icons are not suitable — their style does not fit the garden/nature aesthetic of this app. The correct approach is a **small hand-crafted SVG sprite** built specifically for this app.
...
**Icons needed and their design intent:**

| ID | Usage | Design description |
|---|---|---|
| `icon-home` | Home button | Simple house outline, single path |
...
| `icon-erase` | Erase cell | Rectangle with left-pointing notch (backspace) |
...
**Claude Code instruction:** Generate each symbol as a clean 24×24 viewBox SVG path. Use `stroke` not `fill` for outline icons, `stroke-width="1.8"`, `stroke-linecap="round"`, `stroke-linejoin="round"`. This matches the clean rounded aesthetic of the UI.

**App icon (store asset):** Separate task. Create a 1024×1024 SVG:
- Pink/rose background `#e94560` with rounded square shape (Apple clips to rounded rect, Android uses adaptive layers)
- White stylized flower or leaf mark centered
- Export to PNG at: 1024, 512, 192, 144, 96, 72, 48px
- Android adaptive: provide foreground layer (icon on transparent) + background layer (solid `#e94560`)
...
```html
<!-- Hero card structure -->
<a href="sudoku.html" class="hero-card" aria-label="Play Sudoku">
  <div class="hero-icon">🧩</div>
  <div class="hero-title">SUDOKU</div>
  <div class="hero-desc">Fill the 9×9 grid</div>
  <div class="hero-stat" id="sudokuStat"></div>
</a>

<!-- Carousel -->
<div class="games-carousel">
  <div class="carousel-label">More Games</div>
  <div class="carousel-track">
    <!-- Wordle card — disabled -->
...
<!-- Ad slot -->
<div id="ad-slot-bottom" aria-hidden="true"></div>
```
...
### 4.1 Local Storage Schema (write this now, matches future Supabase schema)
...
Redesign the stats modal to match the screenshot aesthetic the user already has (difficulty tiles in a 2×3 grid):
...
Tapping a tile opens the **drill-down view** (replaces "Back" button with modal close):
...
**Fix sort toggle:** Currently broken. On click, re-sort the runs array:
...
*End of spec. Update version number and date at top of file when making changes.*
